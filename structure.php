...
$select_flat = mysqli_query($link, "SELECT * FROM `structure` WHERE `id` = '$id_struct'");
...
<head>
    ...
    <title>Сооружение - <?= $select_flat['name']; ?></title>
</head>
<body>
    ...
    if($_COOKIE['id_group'] == 2) { ?>
        <a href="./delete-structure.php?id=<?= $id_struct; ?>">Удалить сооружение</a>
    <?php } ?>
    <div class="flat-info">
        ...
        <img src="<?= "./" . $select_flat['struct_img']; ?>">
        <p>Цена: <strong><?= $select_flat['price']; ?></strong></p>
        <a href="./approve-struct.php?id=<?= $id_struct; ?>">Согласовать сделку</a>
    </div>
</body>
</html>